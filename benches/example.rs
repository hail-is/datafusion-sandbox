...
fn run_benchmark<F>(bencher: Bencher, f: F)
where
    F: Fn(&SessionContext) -> Result<DataFrame>,
{
    ...
    let physical_plan =
        rt.block_on(async { f(&ctx).unwrap().create_physical_plan().await.unwrap() });
...
fn range_table(bencher: Bencher, batch_size: u32) {
    run_benchmark(bencher, |ctx: &SessionContext| {

...
`CARGO_BIN_EXE_<name>` only for crate tests and benches. That is a constraint, not an
...
A test reaching for a temporary directory outside `src/tests/combiner_run.rs` and `tests/cli.rs`
